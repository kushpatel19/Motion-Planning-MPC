...
for gamma in gamma_values:
    trajectories.append(mpcCBF_simulation(5, gamma))

# Visualization setup
fig, ax = plt.subplots()
fig.suptitle("Comparision b/w MPC-CBF and MPC-DC", fontsize=16)
...
robots = []
traj_lines = []
colors = ['red', 'orange', 'purple', 'cyan', 'magenta', 'lime']
styles = ['--', '--', '--', '-', '-', '-']  # Dashed for MPC-DC, solid for MPC-CBF

labels = [
    f"MPC-DC (N={N})" for N in N_values
] + [
    f"MPC-CBF (N=5, γ={gamma})" for gamma in gamma_values
]
...
ani.save("mpc_cbf_vs_mpc_dc.gif", writer=PillowWriter(fps=10))
plt.show()
